...
# Pruned from ROADMAP.md on 2026-09-04 (fourth prune)

Everything below was moved out of `ROADMAP.md` when its open list was rebuilt against the actual
code and the live database. **Seven items it still listed as open had already shipped** - the
costumes/props exchange, the show-page share button, the poster lightbox, the society checklist
grid, person identity resolution, removable filter chips, and the `match_show_for_edit` fix.

Kept verbatim because the reasoning is worth more than the verdicts: the delegation findings (what
kinds of task a delegated agent can and cannot be trusted with, and why the overlap cross-check is
weaker than it looks), the backlog interrogation and its finding that inferred demand was wrong
five times out of seven, and the security audit of the Antigravity stint.
